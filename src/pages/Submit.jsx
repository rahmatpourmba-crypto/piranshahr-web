...
const categories = [
  { value: 'فروش', label: 'فروش', icon: '💰' }, { value: 'معاوضه غذا', label: 'معاوضه غذا', icon: '🍲' },
  { value: 'رایگان', label: 'رایگان', icon: '🎁' }, { value: 'استخدام', label: 'استخدام', icon: '💼' },
  { value: 'گمشده', label: 'گمشده', icon: '🔴' }, { value: 'پیداشده', label: 'پیداشده', icon: '🟢' },
  { value: 'نوبت خالی', label: 'نوبت خالی', icon: '🗓' },
]
const typeByCategory = { 'فروش': ['فروش'], 'معاوضه غذا': ['معاوضه'], 'رایگان': ['رایگان'], 'استخدام': ['استخدام', 'درخواست نیرو'], 'گمشده': ['گمشده'], 'پیداشده': ['پیداشده'], 'نوبت خالی': ['نوبت خالی'] }
    ...
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      {step === 0 && (
        <div className="space-y-2 animate-fade-up">
          ...
          <div className="grid grid-cols-2 gap-2">
            {categories.map((c) => (
              <button key={c.value} onClick={() => { update('category', c.value); update('type', typeByCategory[c.value]?.[0] || '') }}
      ...
      {step === 1 && (
        <div className="space-y-3 animate-fade-up">
          ...
          <textarea placeholder="توضیحات..." value={form.description} onChange={(e) => update('description', e.target.value)} rows={4} className="input-field resize-none" />
          <div className="grid grid-cols-2 gap-2">
            <input type="text" placeholder="قیمت (تومان)" value={form.price} onChange={(e) => update('price', e.target.value)} dir="ltr" className="input-field text-right" />
      ...
      {step === 3 && (
        <div className="bg-gray-50 rounded-xl p-4 text-xs space-y-2 border border-gray-100 animate-fade-up">
          ...
          <div className="flex justify-between"><span className="text-gray-400">قیمت:</span><span className="font-bold text-gray-700">{form.price || '---'}</span></div>
